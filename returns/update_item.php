...
if(!$sec->isLoggedIn())
{
	...
	if(isset($_GET['item_id']) and isset($_GET['sale_id']) and isset($_GET['row_id']))
	{
		...
		$service_name_value=$row['srvname'];
		$service_cost_value=$row['srvcost'];
		$item_unit_price_value=$row['item_unit_price'];
		$item_tax_percent_value=$row['item_tax_percent'];
		$refundcomment_value=$row['refundcomment'];
	}


$f1=new form('return validateFormOnSubmit(this)','process_update_item.php','POST','sale item','335',$cfg_theme,$lang);
...
$f1->createInputField("<b>Service Name:</b> ",'text','srv_name',"$service_name_value",'24','160');
$f1->createInputField("<b>Service Cost:</b> ",'text','srv_cost',"$service_cost_value",'24','160');
...
$f1->createInputField("<b>Refund Comment:</b> ",'text','refundcomment',"$refundcomment_value",'24','160');
echo "		
