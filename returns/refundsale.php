...
$display->displayTitle("Refunded Processed");

$table_bg=$display->sale_bg;
$num_items=count($_SESSION['items_in_return']);

$temp_item_number=$_GET['upc']; 
...
for($k=0;$k<$num_items;$k++)
{
	...
	$temp_quantity_purchased=$item_info[5];
	$saleID=$item_info[6];
	...
	$temp_item_cost=number_format(($temp_item_unit_price*$temp_quantity_purchased)+$temp_item_tax_percent,2,'.', '');

	$refundtype=$item_info[9];
	$refundunitprice=$item_info[1];
	$refundtax=$item_info[2];
	$refundtotal=$item_info[3];
	...
	if ($refundtype=="P")
	{
	$field_names=array('refundtype','refundunitprice','refundtax','refundtotal','refundcomment','refunddate');
	$field_data=array("$refundtype","$refundunitprice","$refundtax","$refundtotal","$refundcomment","$todaysDate");
	}else{
	$field_names=array('refundtype','refundcomment','refunddate');
	$field_data=array("$refundtype","$refundcomment","$todaysDate");
	}
	...
	$dbf->updatereturn($field_names,$field_data,$sales_items_table,$saleitem_rowid,false);
	...
	echo "<tr><td align='center'><font color='white'>$temp_item_number</font></td>
...
echo "</table><br>
<table border='0' align='center'><tr><td><b>Refund Sub Total: $cfg_currency_symbol$subtotal</b></td></tr>";
echo "<tr><td><b>$lang->tax: $cfg_currency_symbol$final_tax</b></td></tr>";
echo "<tr><td><b>Total Refund Amount: $cfg_currency_symbol$sale_total_cost</b></td></tr>";
...
echo "<tr><td><b></b></td></tr></table></table>";

echo "<br/><br/>";
echo "<tr><b>$lang->saleID: $saleID</b></tr>";
