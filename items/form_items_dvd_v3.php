...
<script type="text/javascript" src="js/ajax.js"></script>
	...
	************************************************************************************************************/	
	...
	var currentClientID=false;
		...
		var valtotalowner=form.totalowner.value;

		var valitemfound=form.itemfound.value;

if (valaction == "insert") {
		var imagesnapmethod = "<?php echo($cfg_imagesnapmethod);?>";
		var enableimageupload_items = "<?php echo($cfg_enableimageupload_dvd);?>";
		...
		var valcatalognumber=form.supplier_catalogue_number.value;
...
var valworkingonid=form.workingon_id.value; 
var valdvdtitleincatalog=form.dvdtitle_in_catalog.value; 
...
self.location='form_items_dvd_v3.php?reloadcat=yes&valitem_number_value=' + valitemnumber
...
+'&valcatalognumber=' + valcatalognumber
+'&valquantity=' + valquantity
+'&valreorderlevel=' + valreorderlevel
+'&valtranid=' + valtranid
+'&valworkingonid=' + valworkingonid
+'&valdvdtitleincatalog=' + valdvdtitleincatalog
...
;
	...
	function getClientData()
	{
		...
		if(clientId.length > 2 && clientId!=currentClientID){
			...
			ajax.requestFile = 'getDVDTitle.php?getClientId='+clientId;	
	...
	function showClientData()
	{
		...
		var formObj = document.forms['dvds'];
		eval(ajax.response);
	}
...
reason += validateDvdtitle(theForm.description);
...
var validateprice=theForm.validateprice.checked;
if (validateprice == true)
{
reason += validateBuy_price(theForm.buy_price);
reason += validateUnit_price(theForm.unit_price);
}else{
theForm.buy_price.value=0;
theForm.unit_price.value=0;
theForm.buy_price.style.background = 'White';
theForm.unit_price.style.background = 'White';
}
...
function validateDvdtitle(fld) {
    var error = "";
 
    if (dvdtitle_value == 'NotFound') {
        fld.style.background = 'Yellow'; 
        
        error = "Enter DVD Title.\n";
    } else if ((fld.value == "") || (fld.value == "NotFound")){
    	   fld.style.background = 'Yellow'; 
        error = "DVD Title not found in catalog Please enter DVD Title.\n";    
    } else {
        fld.style.background = 'White';
    }
    return error;  
...
if(isset($_POST['item_number'])){
  ...
  $res1 = "select * from dvd_catalog where upc = $enteredupc";
  ...
  if ($description_value==""){
      ...
      $dvdtitle_in_catalog_value = 'no';
   }else{  
      $dvdtitle_in_catalog_value = 'yes';
   } 
  ...
  if(mysql_num_rows(mysql_query($check_item_table,$dbf->conn))){
        ...
        $items_table_query="SELECT * FROM $tablename WHERE item_number = $enteredupc";
      	...
      	$buy_price_value = $cfg_dvd_buyprice;
      	$unit_price_value = $cfg_dvd_sellprice;
      	$quantity_value = 1;
      	$reorder_level_value = 0;
...
{
	$display->displayTitle("$lang->adddvd");
...
echo " | ";
echo "<a href=\"form_items_videogame_dvd_v3.php?action=insert&working_on_id=$_GET[working_on_id]&active_trans_id=$_GET[active_trans_id]\">Buy/Add Games</a>";
...
$supplier_catalogue_number_value=$_GET['valcatalognumber'];
...
$workingon_id_value=$_GET['valworkingonid'];
$gamedvdtitle_in_catalog_value=$_GET['valgamedvdtitleincatalog'];
...
$f1=new form('return validateFormOnSubmit(this)',"form_items_dvd_v3.php?working_on_id=$_GET[working_on_id]&active_trans_id=$_GET[active_trans_id]",'POST','dvds_no','400',$cfg_theme,$lang);
}else{
	if ($cfg_processForm_version=="1"){ 
		$f1=new form('return validateFormOnSubmit(this)','process_form_items_dvd_v1.php','POST','dvds_no','400',$cfg_theme,$lang);
	}else if ($cfg_processForm_version=="2"){
		$f1=new form('return validateFormOnSubmit(this)','process_form_items_dvd_v2.php','POST','dvds_no','400',$cfg_theme,$lang);
	}
}
...
$wherefield = ' a.category_id = ' ." '". "$catid"."'".'and  a.category_id = b.id and b.showon_moviespanel = '."'".'Y'. "'" .' and a.activate_article = '."'".'Y'. "'";
...
$validateprice_checked='checked';
$f1->createInputField("Validate price:",'checkbox','validateprice',"Y",'10','160',"$validateprice_checked");
...
if ($cfg_enableimageupload_dvd == "yes" and $cfg_imagesnapmethod == "pc"){
...
echo "
		...
		<input type='hidden' name='dvdtitle_in_catalog' value='$dvdtitle_in_catalog_value'>
