...
class display
{
    ...
    $usertype=$this->idToField("$tableprefix".'users','type',"$userid");
		...
		{
			$result = mysql_query("SELECT * FROM $table ORDER BY $orderby DESC LIMIT 0,25",$this->conn);
        ...
        if($tablename=='items')
			...
			echo "<td align='center'>\n<a href=\"../checks/form_$tablename.php?action=update&id=$row[id]\"><font color='$this->rowcolor_link'>{$this->lang->update}</font></a></td>
			   
				 <td align='center'>\n<a href=\"javascript:decision('{$this->lang->confirmDelete} $table {$this->lang->table}?','../checks/process_form_$tablename.php?action=delete&id=$row[id]&imgnm=$myimage')\"><font color='$this->rowcolor_link'>{$this->lang->delete}</font></a></td>\n</tr>\n\n";
