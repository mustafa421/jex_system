...
<title>Lightbox JS v2.0 | ImagePage</title>

	<link rel="stylesheet" href="/jestore/jeapp/classes/css/lightbox.css" type="text/css" media="screen" />
	
	<script src="/jestore/jeapp/classes/js/prototype.js" type="text/javascript"></script>
	<script src="/jestore/jeapp/classes/js/scriptaculous.js?load=effects,builder" type="text/javascript"></script>
	<script src="/jestore/jeapp/classes/js/lightbox.js" type="text/javascript"></script>
...
class display
{
	...
	function displayManageTable($tableprefix,$tablename,$tableheaders,$tablefields,$wherefield,$wheredata,$orderby)
	{
		...
		elseif($this->getNumRows($table) >50)
		{
			$result = mysql_query("SELECT * FROM $table ORDER BY $orderby DESC LIMIT 0,50",$this->conn);
        ...
        if($tablename=='items')
			...
			echo "<td align='center'>\n<a href=\"form_$tablename.php?action=update&id=$row[id]\"><font color='$this->rowcolor_link'>{$this->lang->update}</font></a></td>
				 ...
				 <td align='center'>\n<a href=\"javascript:decision('{$this->lang->confirmDelete} $table {$this->lang->table}?','process_form_$tablename.php?action=delete&id=$row[id]&imgnm=$myimage')\"><font color='$this->rowcolor_link'>{$this->lang->delete}</font></a></td>
	...
	function displayReportTable($tableprefix,$tablename,$tableheaders,$tablefields,$wherefield,$wheredata,$date1,$date2,$orderby,$subtitle)
	{
		...
		$table="$tableprefix"."item_transactions";
		echo "\n".'<center>';
