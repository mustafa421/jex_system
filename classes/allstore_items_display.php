...
class display
{
	...
	function displayManageTable($tableprefix,$tablename,$tableheaders,$tablefields,$wherefield,$wheredata,$orderby)
	{
		...
		{
			$result = mysql_query("SELECT * FROM $table WHERE unit_price < 0.01  ORDER BY $orderby",$this->conn);
	...
	if ($nextpage <= $total_pages){ 
       ...
       $backpage = $page - 1;
        ...
        if ($wheredata=='outofstock'){
		 echo "<a href='viewallstore_inventory.php?nextrec=yes&totalpages=$total_pages&outofstock=go&page=".$backpage."'><font color=$urllinkcolor>".BackPage."</font></a>";
		 echo " <a><font color=$pagenumcolor>|Page:$page of $total_pages|</font></a>";
		
	      echo "<a href='viewallstore_inventory.php?nextrec=yes&totalpages=$total_pages&outofstock=go&page=".$nextpage."'><font color=$urllinkcolor>".NextPage."</font></a> ";
	     
		 }else if ($wheredata=='reorder'){
		  echo "<a href='viewallstore_inventory.php?nextrec=yes&totalpages=$total_pages&reorder=go&page=".$backpage."'><font color=$urllinkcolor>".BackPage."</font></a>";
		  echo " <a><font color=$pagenumcolor>|Page:$page of $total_pages|</font></a>";
		
	      echo "<a href='viewallstore_inventory.php?nextrec=yes&totalpages=$total_pages&reorder=go&page=".$nextpage."'><font color=$urllinkcolor>".NextPage."</font></a> ";
	     
		 }else if ($wheredata=='zeroprice'){
		  echo "<a href='viewallstore_inventory.php?nextrec=yes&totalpages=$total_pages&zeroprice=go&page=".$backpage."'><font color=$urllinkcolor>".BackPage."</font></a>";
		  echo " <a><font color=$pagenumcolor>|Page:$page of $total_pages|</font></a>";

	      echo "<a href='viewallstore_inventory.php?nextrec=yes&totalpages=$total_pages&zeroprice=go&page=".$nextpage."'><font color=$urllinkcolor>".NextPage."</font></a> ";
		
		}else{
	      echo "<a href='viewallstore_inventory.php?nextrec=yes&totalpages=$total_pages&page=".$backpage."'><font color=$urllinkcolor>".BackPage."</font></a>";
		  echo " <a><font color=$pagenumcolor>|Page:$page of $total_pages|</font></a>";
	
	     echo "<a href='viewallstore_inventory.php?nextrec=yes&totalpages=$total_pages&page=".$nextpage."'><font color=$urllinkcolor>".NextPage."</font></a> ";
	     }
	}
        ...
        if($tablename=='inventory')
		      {
                 ...
                 $articlename=$tablefields[2];
                 $imagetitle=$row[$articlename];
             
             
             $ItemID=$tablefields[0];
             $ItemRowID=$row[$ItemID];
             

			 $Item_result = mysql_query("SELECT * FROM $table WHERE id = $ItemRowID",$this->conn);
             $Item_row = mysql_fetch_assoc($Item_result);
             $article_image=$Item_row['itemimage'];
             $item_storecode=$Item_row['storecode'];

		       
		       }
		      
		       $myimage=$article_image;
			   $scode =$item_storecode;
        
				
				echo "\n<td  align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>$data</font>\n</td>\n";
			}
			
			echo "<td align='center'>\n<a href=\"displayitemimage.php?&itemimage=$myimage&scode=$item_storecode\" rel=\"lightbox\" title=\"$imagetitle\"><font color='$this->rowcolor_link'>ViewImage</font></a></td>\n</tr>\n\n";
